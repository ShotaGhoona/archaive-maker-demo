...
import type { DrawingPageListDTO, DrawingPageDTO } from '@/entities/product/drawing-page/model/dto';
import type { DrawingPageList, DrawingPage } from '@/entities/product/drawing-page/model/entity';
...
function EntityTestTab() {
  ...
  const [dtoResponse, setDtoResponse] = useState<DrawingPageListDTO | null>(null);
  const [entityResponse, setEntityResponse] = useState<DrawingPageList | null>(null);
  ...
  const selectedDTO: DrawingPageDTO | null = dtoResponse?.items[selectedIndex] ?? null;
  const selectedEntity: DrawingPage | null = entityResponse?.items[selectedIndex] ?? null;
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-lg border bg-card p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <Row label="drawing_number" value={selectedDTO.drawing_number} />
                  <Row label="drawing_file_id" value={selectedDTO.drawing_file_id} type="number" />
                  <Row label="page_number" value={selectedDTO.page_number} type="number" />
                  <Row label="leaf_product_name" value={selectedDTO.leaf_product_name} />
                  <Row label="drawing_file_name" value={selectedDTO.drawing_file_name} />
                  <Row label="drawing_category_name" value={selectedDTO.drawing_category_name} />
                  <Row label="customer_name" value={selectedDTO.customer_name} />
                  <Row label="created_by_name" value={selectedDTO.created_by_name} />
                  <Row label="created_at" value={selectedDTO.created_at} highlight />
                  <Row label="updated_at" value={selectedDTO.updated_at} highlight />
                  <Row label="s3_url" value={selectedDTO.s3_url} truncate />
                  <Row label="remarks" value={selectedDTO.remarks} />
                  <Row label="is_shown_similar_search" value={selectedDTO.is_shown_similar_search} type="boolean" />
                </div>
              </div>
              ...
              <div className="rounded-lg border bg-card p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <Row label="drawingNumber" value={selectedEntity.drawingNumber} />
                  <Row label="drawingFileId" value={selectedEntity.drawingFileId} type="number" />
                  <Row label="pageNumber" value={selectedEntity.pageNumber} type="number" />
                  <Row label="leafProductName" value={selectedEntity.leafProductName} />
                  <Row label="drawingFileName" value={selectedEntity.drawingFileName} />
                  <Row label="drawingCategoryName" value={selectedEntity.drawingCategoryName} />
                  <Row label="customerName" value={selectedEntity.customerName} />
                  <Row label="createdByName" value={selectedEntity.createdByName} />
                  <Row
                    label="createdAt"
                  ...
                  <Row label="isShownSimilarSearch" value={selectedEntity.isShownSimilarSearch} type="boolean" />
                </div>
              </div>
            </div>
          )}
          ...
          <details>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <pre className="max-h-96 overflow-auto rounded bg-muted p-3 text-xs">
                  {JSON.stringify(selectedEntity, (key, value) => {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        <p className="text-sm text-muted-foreground">
          React Query Hooks でEntity型を取得し、page-componentsでconfigを組み立て
        </p>
      </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <pre className="max-h-80 overflow-auto rounded bg-muted p-3 text-xs">
              {JSON.stringify(data.items[0], (key, value) => {
                if (value instanceof Date) {
                  return `[Date] ${value.toISOString()}`;
                }
                return value;
      ...
      {/* フィールドメタデータ */}
      <div className="rounded-lg border bg-card p-4">
        <h3 className="mb-2 font-semibold">DRAWING_PAGE_FIELD_META（Entity層）</h3>
        <p className="mb-2 text-xs text-muted-foreground">
...
function Row({
  ...
  return (
    <div className={`flex justify-between gap-2 ${highlight ? 'bg-yellow-50 dark:bg-yellow-950 -mx-2 px-2 py-1 rounded' : ''}`}>
      <span className="font-mono text-muted-foreground">{label}</span>
      <span className={`font-mono ${valueColor} ${truncate ? 'max-w-48 truncate' : ''}`}>
        {displayValue}

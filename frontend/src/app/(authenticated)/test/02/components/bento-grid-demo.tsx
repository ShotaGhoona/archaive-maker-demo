...
// ダミーデータ
const recentItems = [
  { id: 1, type: 'drawing', name: 'DR-2024-001', title: 'メインフレーム', time: '2時間前', status: 'approved' },
  { id: 2, type: 'bom', name: 'BOM-001', title: '製品A構成', time: '3時間前', status: 'review' },
  { id: 3, type: 'drawing', name: 'DR-2024-002', title: 'サブAssembly', time: '5時間前', status: 'draft' },
  { id: 4, type: 'drawing', name: 'DR-2024-003', title: 'カバー部品', time: '1日前', status: 'approved' },
];

const activities = [
  { user: '山田', action: '図面を承認', target: 'DR-2024-001', time: '10分前' },
  { user: '佐藤', action: 'BOMを更新', target: 'BOM-002', time: '30分前' },
  { user: '田中', action: 'コメントを追加', target: 'DR-2024-005', time: '1時間前' },
  { user: '鈴木', action: '顧客を登録', target: '株式会社ABC', time: '2時間前' },
];
  ...
  return (
    <div
      className={cn(
        ...
        hover && 'transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-neutral-200/50 dark:hover:shadow-neutral-900/50',
        className
      )}
    >
      {children}
...
function MetricCard({
  ...
  return (
    <BentoCard className="flex flex-col justify-between">
      <div className={cn('inline-flex h-12 w-12 items-center justify-center rounded-2xl', color)}>
        <Icon className="h-6 w-6 text-white" />
      </div>
      <div className="mt-4">
        <p className="text-sm text-neutral-500">{label}</p>
        <p className="mt-1 text-4xl font-bold tracking-tight text-neutral-900 dark:text-white">{value}</p>
...
function StatusIndicator({ status }: { status: string }) {
  const config = {
    ...
    review: { icon: Clock, color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-500/10' },
    draft: { icon: AlertCircle, color: 'text-neutral-400', bg: 'bg-neutral-100 dark:bg-neutral-800' },
  };
  const { icon: Icon, color, bg } = config[status as keyof typeof config] || config.draft;

  return (
    <div className={cn('inline-flex h-8 w-8 items-center justify-center rounded-full', bg)}>
    ...
    <div className="min-h-full bg-white p-8 dark:bg-neutral-950">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-neutral-500">今日のタスクと最新の更新を確認しましょう</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 rounded-2xl bg-neutral-100 px-4 py-3 dark:bg-neutral-900">
            ...
            />
            <kbd className="rounded-lg bg-neutral-200 px-2 py-1 text-xs text-neutral-500 dark:bg-neutral-800">⌘K</kbd>
          </div>
        </div>
      </div>
      ...
      <div className="grid auto-rows-[180px] grid-cols-4 gap-4">
        ...
        <BentoCard className="col-span-2 row-span-2" hover={false}>
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-neutral-900 dark:text-white">最近のアイテム</h2>
          ...
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-neutral-900 dark:text-white">週間アクティビティ</h2>
              </div>
        ...
        <BentoCard className="col-span-2 row-span-1" hover={false}>
          <div className="flex h-full flex-col">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-neutral-900 dark:text-white">最近のアクティビティ</h2>
            </div>
              ...
              {activities.map((activity, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-neutral-400">{activity.target}</p>
                  </div>
        ...
        <BentoCard className="col-span-1 row-span-1">
          <div className="flex h-full flex-col">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-neutral-900 dark:text-white">クイック</h2>
            </div>
            <div className="mt-4 grid flex-1 grid-cols-2 gap-2">
              {[
                ...
                >
                  <div className={cn('h-8 w-8 rounded-lg', item.color, 'flex items-center justify-center')}>
        ...
        <BentoCard className="col-span-1 row-span-1">
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="mt-3 text-2xl font-bold text-neutral-900 dark:text-white">5件</p>

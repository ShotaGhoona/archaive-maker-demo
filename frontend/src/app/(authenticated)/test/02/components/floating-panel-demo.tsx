...
// ダミーデータ
const drawings = [
  { id: 1, number: 'DR-2024-001', name: 'メインフレーム組立図', customer: '株式会社A', status: '承認済み', updated: '2024/12/15', version: 'v2.1' },
  { id: 2, number: 'DR-2024-002', name: 'サブアセンブリ詳細', customer: '株式会社B', status: 'レビュー中', updated: '2024/12/14', version: 'v1.3' },
  { id: 3, number: 'DR-2024-003', name: 'カバー部品図', customer: '株式会社A', status: 'ドラフト', updated: '2024/12/13', version: 'v1.0' },
  { id: 4, number: 'DR-2024-004', name: '電装配線図', customer: '株式会社C', status: '承認済み', updated: '2024/12/12', version: 'v3.0' },
  { id: 5, number: 'DR-2024-005', name: 'ベースプレート', customer: '株式会社B', status: '承認済み', updated: '2024/12/11', version: 'v1.5' },
  { id: 6, number: 'DR-2024-006', name: '支柱構造図', customer: '株式会社A', status: 'レビュー中', updated: '2024/12/10', version: 'v2.0' },
  { id: 7, number: 'DR-2024-007', name: 'コネクタ配置図', customer: '株式会社D', status: 'ドラフト', updated: '2024/12/09', version: 'v1.0' },
  { id: 8, number: 'DR-2024-008', name: '外装パネル', customer: '株式会社C', status: '承認済み', updated: '2024/12/08', version: 'v2.2' },
];
...
function StatusBadge({ status }: { status: string }) {
  const config = {
    '承認済み': { icon: CheckCircle2, bg: 'bg-emerald-50 dark:bg-emerald-500/10', text: 'text-emerald-600 dark:text-emerald-400' },
    'レビュー中': { icon: Clock, bg: 'bg-amber-50 dark:bg-amber-500/10', text: 'text-amber-600 dark:text-amber-400' },
    'ドラフト': { icon: AlertCircle, bg: 'bg-neutral-100 dark:bg-neutral-800', text: 'text-neutral-500' },
  };
  const { icon: Icon, bg, text } = config[status as keyof typeof config] || config['ドラフト'];
...
function FilterPanel({
  ...
  setFilters: (filters: { status: string; customer: string; dateRange: string }) => void;
}) {
  if (!open) return null;
      ...
      <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-2xl dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <div className="flex items-center justify-between border-b border-neutral-100 px-4 py-3 dark:border-neutral-800">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-neutral-900 dark:text-white">フィルター</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between border-t border-neutral-100 px-4 py-3 dark:border-neutral-800">
          <button
            onClick={() => setFilters({ status: 'すべて', customer: 'すべて', dateRange: 'すべて' })}
...
export function FloatingPanelDemo() {
  ...
  const filteredDrawings = drawings.filter((d) => {
    if (filters.status !== 'すべて' && d.status !== filters.status) return false;
    if (filters.customer !== 'すべて' && d.customer !== filters.customer) return false;
    if (searchQuery && !d.name.includes(searchQuery) && !d.number.includes(searchQuery)) return false;
    return true;
  });

  const activeFiltersCount = [filters.status, filters.customer, filters.dateRange].filter(
    (f) => f !== 'すべて'
  ).length;
    ...
    <div className="flex min-h-full flex-col bg-white dark:bg-neutral-950">
      ...
      <div className="border-b border-neutral-200 dark:border-neutral-800">
        <div className="flex items-center gap-1 px-6">
          {[
            ...
            { icon: FolderOpen, label: 'プロジェクト', active: false, count: 45 },
          ].map((tab) => (
            <button
              key={tab.label}
              className={cn(
      ...
      {selectedRows.length > 0 && (
        <div className="flex items-center justify-between bg-neutral-900 px-6 py-3 text-white dark:bg-white dark:text-neutral-900">
          <span className="text-sm font-medium">{selectedRows.length}件選択中</span>
          <div className="flex items-center gap-2">
          ...
          <thead className="sticky top-0 bg-neutral-50 dark:bg-neutral-900">
            <tr className="border-b border-neutral-200 dark:border-neutral-800">
              <th className="px-6 py-3 text-left">
                <input
                  type="checkbox"
                  checked={selectedRows.length === filteredDrawings.length && filteredDrawings.length > 0}
          ...
          <tbody>
            {filteredDrawings.map((drawing) => (
              ...
              >
                ...
                <td className="px-4 py-4">
                  <span className="text-sm text-neutral-900 dark:text-white">{drawing.name}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm text-neutral-500">{drawing.customer}</span>
                </td>
                <td className="px-4 py-4">
                  <StatusBadge status={drawing.status} />
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm text-neutral-500">{drawing.updated}</span>
                </td>
                <td className="px-4 py-4">

...
import { Info, FileText, ZoomIn, ZoomOut, RotateCw } from 'lucide-react';
...
import type { Document } from '@/shared/dummy-data/bom-v2';
  ...
  if (!document) {
    return (
      <Card className='flex min-h-0 flex-1 flex-col items-center justify-center gap-0'>
        <FileText className='mb-4 size-16 text-muted-foreground/50' />
        <p className='text-lg font-medium text-muted-foreground'>
          帳票を選択してください
        </p>
        <p className='mt-1 text-sm text-muted-foreground'>
          左のリストから帳票を選択するとプレビューが表示されます
        </p>
      </Card>
    );
  }
    ...
    <Card className='relative flex min-h-0 flex-1 flex-col gap-0 overflow-hidden'>
      ...
      <div className='absolute right-3 top-3 z-10 flex items-center gap-2'>
        ...
        <Tooltip>
          ...
          <TooltipContent>帳票情報</TooltipContent>
        </Tooltip>
      </div>
      ...
      <div className='flex min-h-0 flex-1 items-center justify-center overflow-auto bg-muted/30 p-8'>
        {document.s3Path ? (
          ...
          <div className='flex flex-col items-center justify-center text-muted-foreground'>
            <FileText className='mb-4 size-24 opacity-50' />
            <p className='text-lg font-medium'>プレビューがありません</p>
            <p className='mt-1 text-sm'>この帳票にはプレビュー画像が設定されていません</p>
          </div>
        )}
      </div>
    </Card>

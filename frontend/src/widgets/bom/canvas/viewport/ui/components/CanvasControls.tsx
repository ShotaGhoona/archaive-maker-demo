...
const HELP_CONTENT = [
  ...
  { action: '移動', mouse: '中クリック + ドラッグ', trackpad: '2本指スクロール' },
          ...
          <table className="text-xs">
            <tbody>
              {HELP_CONTENT.map((row, index) => (
                <tr key={index}>
                  <td className="pr-3 py-0.5 font-medium text-gray-600">{row.action}</td>
                  <td className="pr-3 py-0.5 text-gray-500">{row.mouse}</td>
